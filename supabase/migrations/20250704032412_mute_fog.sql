/*
  ...
  1. Functions Updated
    - `increment_plans_generated` - Fixed parameter name to `input_user_id`
    - `increment_content_generated` - Fixed parameter name to `input_user_id`
    - `is_user_admin` - Ensured consistent parameter naming
    - `get_user_stats` - Ensured consistent parameter naming
    - `check_user_limits` - Ensured consistent parameter naming

  2. Changes Made
    - Used CREATE OR REPLACE FUNCTION to safely update existing functions
    - Standardized all parameter names to use `input_user_id` prefix
...
CREATE OR REPLACE FUNCTION increment_content_generated(input_user_id uuid, input_amount integer DEFAULT 1)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  VALUES (input_user_id, input_amount)
  ON CONFLICT (id)
  DO UPDATE SET 
    content_generated_this_month = COALESCE(subscriptions.content_generated_this_month, 0) + input_amount;
END;
$$;

-- Ensure is_user_admin function is properly defined with consistent naming
...
-- Ensure get_user_stats function is properly defined with consistent naming
...
BEGIN
  ...
  SELECT is_user_admin(input_user_id) INTO user_is_admin;
  ...
  SELECT 
    ...
    CASE 
      WHEN user_is_admin THEN -1  -- Unlimited for admins
...
-- Ensure check_user_limits function is properly defined with consistent naming
CREATE OR REPLACE FUNCTION check_user_limits(input_user_id uuid, input_check_type text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Check based on type
  IF input_check_type = 'plans' THEN
  ...
  ELSIF input_check_type = 'content' THEN
...
COMMENT ON FUNCTION increment_plans_generated(uuid) IS 'Increments the monthly plans counter for a user';
COMMENT ON FUNCTION increment_content_generated(uuid, integer) IS 'Increments the monthly content counter for a user by specified amount';
COMMENT ON FUNCTION is_user_admin(uuid) IS 'Checks if user has admin privileges (email=admin@univoice.ro or role=admin)';
COMMENT ON FUNCTION get_user_stats(uuid) IS 'Returns comprehensive user statistics with plan limits (-1 for unlimited admin access)';
COMMENT ON FUNCTION check_user_limits(uuid, text) IS 'Checks if user is within their plan limits (admin always returns true)';