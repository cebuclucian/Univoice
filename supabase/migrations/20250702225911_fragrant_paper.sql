/*
  ...
  1. New Functions
    - `get_user_stats(user_id)` - Returns comprehensive user statistics including plan counts, limits, and subscription info
    - `check_user_limits(user_id, check_type)` - Checks if user has reached their monthly limits
    - `increment_plans_generated(user_id)` - Increments the monthly plans counter
    - `increment_content_generated(user_id, amount)` - Increments the monthly content counter

  2. Security
    - Functions are accessible to authenticated users
    - Each function validates user access through RLS policies
...
CREATE OR REPLACE FUNCTION public.increment_plans_generated(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    ...
    INSERT INTO public.subscriptions (id, plan, status, plans_generated_this_month, content_generated_this_month)
    VALUES (p_user_id, 'free', 'active', 1, 0)
    ON CONFLICT (id) 
    DO UPDATE SET 
...
CREATE OR REPLACE FUNCTION public.increment_content_generated(p_user_id uuid, amount integer DEFAULT 1)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    -- Insert or update subscription record
    INSERT INTO public.subscriptions (id, plan, status, plans_generated_this_month, content_generated_this_month)
    VALUES (p_user_id, 'free', 'active', 0, amount)
    ON CONFLICT (id) 
    DO UPDATE SET 
