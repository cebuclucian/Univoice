/*
  ...
  2. Security
    - Functions use security definer to access data with proper RLS
    - Input validation for user_id parameter
  
  3. Features
    - Extracts brand voice data from marketing plans
    - Provides historical tracking of brand voice changes
    - Analytics for brand voice evolution patterns
*/
  ...
  SELECT 
    ...
    COALESCE(
      jsonb_build_object(
        ...
        'brand_profile_updated_at', bp.updated_at::text
      ),
      '{}'::jsonb
    ) as brand_voice_data
