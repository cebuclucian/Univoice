...
export const MarketingPlanDetails: React.FC<MarketingPlanDetailsProps> = ({
  ...
  // Helper function to safely get nested data with multiple possible paths
  const getNestedData = (obj: any, paths: string[]): any => {
  ...
  // Helper function to get calendar data from multiple possible structures
  const getCalendarData = () => {
    return getNestedData(plan.details, [
      'editorial_calendar',
      'content_calendar',
      'calendar',
      'weekly_content',
  ...
  // Helper function to get platforms data from multiple possible structures
  ...
  // Helper function to get KPIs data from multiple possible structures
  const getKPIsData = () => {
    return getNestedData(plan.details, [
      'kpis_smart',
      'kpis',
  ...
  // Helper function to get objectives data
  ...
  // Helper function to get target audience data
  const getTargetAudienceData = () => {
    return getNestedData(plan.details, [
      'target_audience',
      'audience',
      'target_market',
  ...
  // Helper function to get strategy data
  ...
  const generateTextExport = () => {
    ...
    // Target Audience
    const targetAudience = getTargetAudienceData();
    text += 'AUDIENȚA ȚINTĂ:\n';
    if (targetAudience) {
      text += `Principală: ${targetAudience.primary || 'N/A'}\n`;
      if (targetAudience.demographics) {
        text += `Demografie: ${targetAudience.demographics}\n`;
      }
      if (targetAudience.pain_points && targetAudience.pain_points.length > 0) {
        text += 'Puncte de durere:\n';
        targetAudience.pain_points.forEach((point: string, i: number) => {
          text += `- ${point}\n`;
        });
      }
    } else {
  ...
  const generateCalendarExport = () => {
    ...
    if (calendar && Array.isArray(calendar) && calendar.length > 0) {
      calendar.forEach((week: any) => {
        const weekNumber = week.week_number || week.week || 'Necunoscut';
        text += `SĂPTĂMÂNA ${weekNumber}\n`;
        text += '='.repeat(20) + '\n\n';
        
        // Get posts from multiple possible structures
        const posts = week.posts || week.content || week.items || [];
        ...
        if (posts && Array.isArray(posts) && posts.length > 0) {
          posts.forEach((post: any) => {
            ...
            const content = post.main_text || post.description || post.content || 'Fără conținut';
...
Conținut: ${post.main_text || post.description || post.content || 'Fără conținut'}
        ...
        <Card className="shadow-lg" animation="slideInLeft">
          ...
          <div className="prose max-w-none">
            <p className="text-gray-700 leading-relaxed mb-6">
              {plan.details?.summary || plan.details?.description || 'Plan de marketing personalizat generat cu AI.'}
            </p>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="shadow-lg" animation="slideInLeft" hover="subtle">
            ...
            <div className="space-y-3">
              {objectives && Array.isArray(objectives) && objectives.length > 0 ? (
                ...
                <p className="text-gray-500 italic">Nu sunt definite obiective specifice</p>
              )}
            </div>
          </Card>
          ...
          <Card className="shadow-lg" animation="slideInRight" hover="subtle">
            ...
            {targetAudience ? (
              <div className="space-y-3 text-sm">
                {targetAudience.primary && (
                  <div>
                    ...
                    <p className="text-gray-700">{targetAudience.primary}</p>
                  </div>
                )}
                ...
                {targetAudience.demographics && (
                  <div>
                    ...
                    <p className="text-gray-700">{targetAudience.demographics}</p>
                  </div>
                )}
                
                {targetAudience.pain_points && Array.isArray(targetAudience.pain_points) && targetAudience.pain_points.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-gray-800 mb-1">Puncte de durere:</h4>
                    <ul className="space-y-1">
                      {targetAudience.pain_points.map((point: string, index: number) => (
              ...
              <p className="text-gray-500 italic">Nu sunt definite detalii despre audiența țintă</p>
            )}
          </Card>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {strategy.positioning && (
                <div>
                  ...
                  <p className="text-gray-700 text-sm">{strategy.positioning}</p>
                </div>
              )}
              
              {strategy.key_messages && Array.isArray(strategy.key_messages) && strategy.key_messages.length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-800 mb-2">Mesaje cheie:</h4>
                  <ul className="space-y-1">
                    {strategy.key_messages.map((message: string, index: number) => (
              ...
              {strategy.content_pillars && Array.isArray(strategy.content_pillars) && strategy.content_pillars.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {strategy.content_pillars.map((pillar: string, index: number) => (
                      <span 
                        key={index}
          ...
          <Card className="shadow-lg" animation="fadeInUp" hover="subtle">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {platforms.map((platform: any, index: number) => {
                const platformName = platform.name || platform.platform || `Platform ${index + 1}`;
                return (
                  <Card 
                    key={index}
                    className={`border-l-4 ${getPlatformColor(platformName).includes('blue') ? 'border-blue-400' : 
                      getPlatformColor(platformName).includes('pink') ? 'border-pink-400' :
                      getPlatformColor(platformName).includes('sky') ? 'border-sky-400' :
                      getPlatformColor(platformName).includes('indigo') ? 'border-indigo-400' :
                      'border-gray-400'
                    }`}
                    padding="sm"
                    hover="subtle"
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      <div className={`p-2 rounded-lg ${getPlatformColor(platformName)}`}>
                    ...
                    <div className="space-y-2 text-sm">
                      <p className="text-gray-700">{platform.strategy || 'Strategie nespecificată'}</p>
          ...
          {calendar && Array.isArray(calendar) && calendar.length > 0 ? (
            <div className="space-y-6">
              {calendar.map((week: any, weekIndex: number) => {
                const weekNumber = week.week_number || week.week || `Săptămâna ${weekIndex + 1}`;
                const posts = week.posts || week.content || week.items || [];
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-bold text-gray-900">
                        {typeof weekNumber === 'string' ? weekNumber : `Săptămâna ${weekNumber}`}
                      </h4>
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => {
                          const weekContent = posts.map((post: any) => {
                            ...
                            const title = post.title || 'Fără titlu';
                            const content = post.main_text || post.description || post.content || 'Fără conținut';
                            const hashtags = post.hashtags && Array.isArray(post.hashtags) ? post.hashtags.join(' ') : '';
                            const cta = post.call_to_action || '';
                            
                            return `${platform} (${contentType}): ${title}\n${content}\n${hashtags}\n${cta}`;
                      ...
                      {posts && Array.isArray(posts) && posts.length > 0 ? (
                        posts.map((post: any, postIndex: number) => {
                          ...
                          return (
                            <Card 
                              key={postIndex}
                              className={`border-l-4 ${getPlatformColor(platform)}`}
                              padding="sm"
                              hover="subtle"
                            >
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <Button 
                                  variant="ghost" 
                                  size="sm"
                                  onClick={() => copyToClipboard(content, `content-${weekIndex}-${postIndex}`)}
          ...
          {platforms && Array.isArray(platforms) && platforms.length > 0 ? (
            <div className="space-y-8">
              {platforms.map((platform: any, platformIndex: number) => {
                const platformName = platform.name || platform.platform || `Platform ${platformIndex + 1}`;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="bg-white p-4 rounded-lg border border-gray-100 mb-4">
                      ...
                      <p className="text-gray-700 text-sm">{platform.strategy || 'Strategie nespecificată'}</p>
                    </div>
                    ...
                    {calendar && Array.isArray(calendar) && (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {calendar
                            .flatMap((week: any) => week.posts || week.content || week.items || [])
                            .filter((post: any) => {
                              const postPlatform = (post.platform || '').toLowerCase();
                              const targetPlatform = platformName.toLowerCase();
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Button 
                                    ...
                                    onClick={() => {
                                      const content = post.main_text || post.description || post.content || 'Fără conținut';
                                      copyToClipboard(content, `platform-content-${platformIndex}-${contentIndex}`);
                                    }}
                                    className="p-1 h-6 w-6"
                                  >
                                ...
                                <p className="text-xs text-gray-600 line-clamp-3">
                                  {post.main_text || post.description || post.content || 'Fără conținut'}
                                </p>
                              </Card>
                            ))
                          }
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 text-sm">
                      {kpi.description || kpi.measurement || 'Fără descriere'}
                    </p>
